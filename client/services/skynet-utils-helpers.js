angular.module('angular-skynet').factory('skynetHelpers', function($meteor, $rootScope, uiGridConstants) {
    var factory = {};
    ...
    factory.helpers = {
        ...
        initUIGrid: function(scope) {
            ...
            scope.uiGrid.setAllColumnsProp('headerCellClass', scope.uiGrid.highlightFilteredHeader);
            scope.uiGrid.setAllColumnsProp('enablePinning', true);
            scope.uiGrid.setAllColumnsProp('groupingShowAggregationMenu', false);

            scope.uiGrid.gridOptions.columnDefs = scope.uiGrid.columns;
        },
        initNewUiGridStateParams: function(scope) {
            scope.newState = {
        ...
        validateUiGridStateForm: function(state) {
            console.log('state: ', state)
            let error = {};
            if (!state.ten) {
                error.message = "Chưa có thông tin về tên trạng thái.";
                return error;
            }
            return;
        },
        buildNewUiGridState: function(newState, gridApi) {
            newState.gia_tri = gridApi.saveState.save();
            if (!newState.metadata)
                newState.metadata = {};
            this.buildMetadata('buildNew', newState.metadata);
        },
